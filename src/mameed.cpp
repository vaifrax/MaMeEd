...
int main(int argc, char *argv[]) {
	...
	//mwin->show(argc, argv);
	mwin->show(1, argv); // do not pass on command line parameters (except exec path)
	int retVal = Fl::run();
	delete mwin; // clean up
	return retVal;
}
...
class MWin : protected Fl_Window {
  public:
	Fl_Window* about_dlg;
};

void options_cb(Fl_Widget*, void*) {}
...
Fl_Menu_Item menuitems[] = {
	{ "&File", 0, 0, 0, FL_SUBMENU },
		...
		{ "&Open File...", FL_COMMAND + 'o', (Fl_Callback *)options_cb },
		{ "&Insert File...", FL_COMMAND + 'i', (Fl_Callback *)options_cb, 0,
		FL_MENU_DIVIDER },
		{ "&Save File", FL_COMMAND + 's', (Fl_Callback *)options_cb },
		{ "Save File &As...", FL_COMMAND + FL_SHIFT + 's', (Fl_Callback *)options_cb
		, 0, FL_MENU_DIVIDER },
		{ "New &View", FL_ALT + 'v', (Fl_Callback *)options_cb, 0 },
		{ "&Close View", FL_COMMAND + 'w', (Fl_Callback *)options_cb, 0,
		FL_MENU_DIVIDER },
		{ "E&xit", FL_COMMAND + 'q', (Fl_Callback *)options_cb, 0 },
		{ 0 },
	{ "&Edit", 0, 0, 0, FL_SUBMENU },
		{ "&Undo", FL_COMMAND + 'z', (Fl_Callback *)options_cb, 0, FL_MENU_DIVIDER },
		{ "Cu&t", FL_COMMAND + 'x', (Fl_Callback *)options_cb },
		{ "&Copy", FL_COMMAND + 'c', (Fl_Callback *)options_cb },
		{ "&Paste", FL_COMMAND + 'v', (Fl_Callback *)options_cb },
		{ "&Delete", 0, (Fl_Callback *)options_cb },
		{ 0 },
	{ "&Search", 0, 0, 0, FL_SUBMENU },
		{ "&Find...", FL_COMMAND + 'f', (Fl_Callback *)options_cb },
		{ "F&ind Again", FL_COMMAND + 'g', options_cb },
		{ "&Replace...", FL_COMMAND + 'r', options_cb },
		{ "Re&place Again", FL_COMMAND + 't', options_cb },
		{ 0 },
	{ "&Help", 0, 0, 0, FL_SUBMENU },
		{ "&About...", FL_COMMAND + 'a', (Fl_Callback *)about_cb },
		{ 0 },
...
int main(int argc, char **argv) {
	MWin *window = new MWin(800,800);

	Fl_Menu_Bar *m = new Fl_Menu_Bar(0, 0, 640, 30);
	m->copy(menuitems);

	Fl_Input* pathTextEdit = new Fl_Input(20, 50, 270, 24, "");
	pathTextEdit->value("C:\\");

	Fl_Button* pathButton = new Fl_Button(300, 50, 34, 24, "Path"); // "@#menu"
	pathButton->type(FL_NORMAL_BUTTON);

	//Fl_Box *box = new Fl_Box(20,400,300,100,"Hello, World!");
	//box->box(FL_UP_BOX);
	...
	Fl_Scrollbar* fileScrollbar = new Fl_Scrollbar(300, 80, 20, 400, "");
	Fl_Scrollbar* dataScrollbar = new Fl_Scrollbar(560, 40, 20, 600, "");

	window->end();

window->about_dlg = new Fl_Window(300, 105, "Replace");
